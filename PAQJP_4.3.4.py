...
def encode_base256_datetime(dt: datetime = None) -> bytes:
    """Encode file creation timestamp into 5 bytes using base-256 (Unix timestamp scaled to 40 bits)."""
    if dt is None:
        logging.warning("No datetime provided, using current time as fallback. Should use file creation time.")
        dt = datetime.now()
    # Convert to Unix timestamp (seconds since epoch)
    timestamp = int(dt.timestamp())
    # Scale to 40 bits (0 to 2^40 - 1, max ~1.099 trillion seconds)
    max_timestamp = 2**40 - 1
    scaled_value = min(timestamp, max_timestamp)  # Cap at 40 bits
    # Convert to 5 base-256 bytes
    return bytes([(scaled_value >> (i * 8)) & 0xFF for i in range(4, -1, -1)])

def decode_base256_datetime(data: bytes) -> datetime:
    """Decode 5 bytes of base-256 timestamp back to datetime."""
    if len(data) != 5:
        logging.error("Insufficient data for datetime decoding")
        raise ValueError("Data must be 5 bytes")
    # Reconstruct the 40-bit value
    value = sum((data[i] << (8 * (4 - i))) for i in range(5))
    # Convert back to timestamp and datetime
    timestamp = min(value, 2**40 - 1)  # Ensure within 40-bit range
    return datetime.fromtimestamp(timestamp)

def transform_with_prime_xor_every_3_bytes(data, creation_time: datetime = None):
    ...
    # Prepend 5-byte base-256 encoded creation date
    date_bytes = encode_base256_datetime(creation_time)
    return date_bytes + bytes(transformed)

def transform_with_pattern_chunk(data, creation_time: datetime = None):
...
class SmartCompressor:
    ...
    def transform_01(self, data, creation_time: datetime = None):
        transformed = bytearray(data)
        for prime in PRIMES:
            xor_val = prime if prime == 2 else max(1, math.ceil(prime * 4096 / 28672))
            for _ in range(7):
    ...
    def reverse_transform_01(self, data):
        if len(data) < 5:
            logging.error("Insufficient data for reverse transformation (missing date bytes)")
            return b'', None
        date_bytes = data[:5]
        creation_date = decode_base256_datetime(date_bytes)
        transformed_data = data[5:]
        return transformed_data, creation_date
    
    def transform_03(self, data, creation_time: datetime = None):
        transformed = bytearray()
        for i in range(0, len(data), 4):
            chunk = data[i:i + 4]
            transformed.extend([b ^ 0xFF for b in chunk])
    ...
    def transform_04(self, data, creation_time: datetime = None, repeat=50):
        transformed = bytearray(data)
        for _ in range(repeat):
    ...
    def reverse_transform_04(self, data):
        ...
        date_bytes = data[:5]
        creation_date = decode_base256_datetime(date_bytes)
        transformed_data = bytearray(data[5:])
        for _ in range(50):
            for i in range(len(transformed_data)):
                transformed_data[i] = (transformed_data[i] + (i % 256)) % 256
        return bytes(transformed_data), creation_date
    
    def transform_05(self, data, creation_time: datetime = None, shift=3):
        transformed = bytearray(data)
        for i in range(len(transformed)):
    ...
    def reverse_transform_05(self, data, shift=3):
        ...
        transformed_data = bytearray(data[5:])
        for i in range(len(transformed_data)):
            transformed_data[i] = ((transformed_data[i] >> shift) | (transformed_data[i] << (8 - shift))) & 0xFF
        return bytes(transformed_data), creation_date
    
    def transform_06(self, data, creation_time: datetime = None, seed=42):
        random.seed(seed)
    ...
    def reverse_transform_06(self, data):
        ...
        if not transformed_data:
            logging.error("Transformed data is empty after date extraction")
            return b'', creation_date
        random.seed(42)  # Match the seed from transform_06
        substitution = list(range(256))
        random.shuffle(substitution)
        ...
        for i in range(len(transformed_data)):
            if transformed_data[i] >= 256:  # Validate input range
                logging.error(f"Invalid byte value at index {i}: {transformed_data[i]}")
                return b'', creation_date
            transformed_data[i] = reverse_substitution[transformed_data[i]]
        return bytes(transformed_data), creation_date

    def transform_07(self, data, creation_time: datetime = None):
        """
    ...
    def reverse_transform_07(self, data):
        ...
        transformed_data = bytearray(data[5:])
        pi_length = len(self.PI_DIGITS)
        data_size_kb = len(transformed_data) / 1024
        cycles = min(10, max(1, int(data_size_kb)))
        logging.info(f"reverse_transform_07: Using {cycles} cycles for {len(transformed_data)} bytes (base-256)")

        # Reverse pi-based XOR with dynamic cycles
        for _ in range(cycles):
            for i in range(len(transformed_data)):
                pi_digit = self.PI_DIGITS[i % pi_length]
                transformed_data[i] ^= pi_digit

        # Reverse pre-transformation
        size_byte = len(transformed_data) % 256
        for i in range(len(transformed_data)):
            transformed_data[i] ^= size_byte

        # Reverse circular shift
        shift = len(transformed_data) % pi_length
        self.PI_DIGITS = self.PI_DIGITS[-shift:] + self.PI_DIGITS[:-shift]

        return bytes(transformed_data), creation_date

    def transform_08(self, data, creation_time: datetime = None):
        """
    ...
    def reverse_transform_08(self, data):
        ...
        data_size_kb = len(transformed_data) / 1024
        cycles = min(10, max(1, int(data_size_kb)))
        logging.info(f"reverse_transform_08: Using {cycles} cycles for {len(transformed_data)} bytes (base-256)")
        ...
        # Reverse pre-transformation
        size_prime = find_nearest_prime_around(len(transformed_data) % 256)
        for i in range(len(transformed_data)):
            transformed_data[i] ^= size_prime
    ...
    def compress_with_best_method(self, data, filetype, input_file):
        # Use file creation time
        try:
            creation_time = datetime.fromtimestamp(os.stat(input_file).st_ctime)
        except (OSError, AttributeError) as e:
            logging.warning(f"Failed to get creation time from {input_file}: {e}. Using current time as fallback.")
            creation_time = datetime.now()
        
        # Line 670: Define transformations with only 01-08, including creation date
        transformations = [
            (1, self.transform_01),
            (2, self.transform_03),
            (3, self.transform_04),
        ...
        if filetype in [Filetype.JPEG, Filetype.TEXT]:
            transformations = [(6, self.transform_07), (7, self.transform_08)] + transformations
        ...
        for marker, transform in transformations:
            transformed = transform(data, creation_time)
        ...
        return bytes([best_marker]) + best_compressed  # Creation date already in transformed data
    
    def decompress_with_best_method(self, data):
        if len(data) < 1:  # Minimum: 1 byte marker
            logging.error("Empty or insufficient compressed data.")
            return b'', None
        
        method_marker = data[0]
        compressed_data = data[1:]  # Creation date is within transformed data
        
        reverse_transforms = {
            1: self.reverse_transform_01,
            2: self.reverse_transform_03,
            3: self.reverse_transform_04,
            4: self.reverse_transform_05,
            5: self.reverse_transform_06,
            6: self.reverse_transform_07,
            7: self.reverse_transform_08,
        }
        ...
        if method_marker == 4:
            ...
            try:
                ...
                if len(hex_str) % 2 != 0:
                    hex_str = '0' + hex_str
                decompressed_data = binascii.unhexlify(hex_str)
                if len(decompressed_data) < 5:
                    logging.error("Decompressed data too short to extract date")
                    return decompressed_data, None
                date_bytes = decompressed_data[:5]
                creation_date = decode_base256_datetime(date_bytes)
                return decompressed_data[5:], creation_date
            except Exception as e:
        ...
        try:
            ...
            result, creation_date = reverse_transforms[method_marker](decompressed)
...
def main():
    ...
    print("Options:")
    print("1 - Compress file (PAQJP_4 with transformations and creation date)")
    print("2 - Decompress file (PAQJP_4 with transformations and creation date)")
    ...
    if not input_file:
        logging.error("No input file name provided.")
        return
    ...
    if choice == '1':
        ...
        try:
            decompressed, creation_date = compressor.decompress_with_best_method(input_data)
            if decompressed is None:
                return
            
            # Preserve original extension based on marker (6 or 7 for .jpg/.txt), but creation date is independent of name
            orig_ext = os.path.splitext(input_file)[1].lower()
            if filetype in [Filetype.JPEG, Filetype.TEXT] and not output_file.endswith(('.jpg', '.jpeg', '.txt')):
            ...
            decomp_size = os.path.getsize(output_file) if os.path.exists(output_file) else len(decompressed)
            ...
            if creation_date:
                logging.info(f"Restored Creation Date: {creation_date.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
